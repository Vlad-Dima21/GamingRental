...
public abstract class BaseControllerMYSQLTests {
    ...
    void retrieveAdminToken(AdminDTO adminDTO) throws Exception {
        String AUTH_ADMIN = "/api/auth/admin/login";
        adminToken = fetchToken(AUTH_ADMIN, StringifyJSON.toJSON(adminDTO));
    }
    void retrieveUserToken(UserDTO userDTO) throws Exception {
        String AUTH_USER = "/api/auth/login";
